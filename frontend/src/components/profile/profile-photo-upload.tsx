...
export function ProfilePhotoUpload({
  ...
  return (
    <section className="mb-8 rounded-xl border border-outline-variant bg-surface p-6">
      <h2 className="text-lg font-semibold text-on-surface">{t('photo')}</h2>

      <div className="mt-4 flex flex-col items-start gap-4 sm:flex-row sm:items-center">
        <div className="relative h-24 w-24 overflow-hidden rounded-full border-2 border-outline-variant bg-surface-container-low">
          {preview ? (
            <Image
              src={preview}
              alt={t('photoAlt')}
            ...
            <div className="flex h-full w-full items-center justify-center text-2xl text-on-surface-variant">
              ?
            </div>
          )}
          {isUploading ? (
      ...
      {validationError ? (
        <p className="mt-3 text-sm text-amber-700 dark:text-amber-400">{validationError}</p>
      ) : null}
