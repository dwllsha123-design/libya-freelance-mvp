...
export default function EditProjectPage() {
  ...
  useEffect(() => {
    Promise.all([api.getManage(params.id), api.loadFormData()])
      ...
      .catch(() => setError(tDashboard('projectNotFound')))
      .finally(() => setIsLoading(false));
  }, [api, params.id, tDashboard]);
  ...
  if (error || !project) {
    return <div className="p-8 text-center text-red-600">{error ?? tDashboard('notFound')}</div>;
  }
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-10">
      ...
      <h1 className="mt-4 text-3xl font-bold text-on-surface">{t('editProject')}</h1>

      {readOnly ? (
        <p className="mt-4 rounded-lg bg-amber-50 p-3 text-sm text-amber-800">
          {tDashboard('readOnlyProject')}
        </p>
      ) : null}
