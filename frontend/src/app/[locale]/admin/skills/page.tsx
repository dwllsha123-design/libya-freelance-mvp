  ...
  return (
    <div className="space-y-6">
      ...
      <form onSubmit={(e) => void onCreate(e)} className="flex flex-wrap gap-2 rounded-xl border bg-white p-4">
        <input value={name} onChange={(e) => setName(e.target.value)} placeholder={t('skillName')} className="rounded border px-3 py-2 text-sm" required />
      ...
      <div className="space-y-2">
        {isLoading ? (
        ...
        ) : !data?.items.length ? <AdminEmptyState message={t('noSkills')} /> : data.items.map((s) => (
          <div key={String(s.id)} className="flex flex-wrap items-center justify-between gap-2 rounded-xl border bg-white p-4">
            ...
            <div className="flex items-center gap-2">
              <StatusBadge label={s.isActive ? t('activeF') : t('inactiveF')} tone={s.isActive ? 'success' : 'danger'} />
              {s.isActive ? (
                <button type="button" onClick={() => void api.deactivateSkill(String(s.id)).then(reload)} className="text-sm text-red-600">{t('deactivate')}</button>
              ) : (
                <button type="button" onClick={() => void api.activateSkill(String(s.id)).then(reload)} className="text-sm text-primary">{t('activate')}</button>
              )}
            </div>
          </div>
        ))}
