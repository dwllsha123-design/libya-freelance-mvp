  ...
  return (
    <div>
      ...
      <div className="mt-6 space-y-3">
        {!data?.items.length ? <AdminEmptyState message={t('noReviews')} /> : data.items.map((review) => (
          <div key={String(review.id)} className="rounded-xl border bg-white p-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <StatusBadge
                label={review.isVisible ? t('visible') : t('hidden')}
      ...
      <AdminConfirmDialog
        open={pending !== null}
        title={pending?.action === 'hide' ? t('hideReviewTitle') : t('restoreReviewTitle')}
        message={tUi('confirmMessage')}
