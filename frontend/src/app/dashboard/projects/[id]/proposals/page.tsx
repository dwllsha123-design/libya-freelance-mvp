...
export default function ProjectProposalsPage() {
  ...
  async function executeAction() {
    ...
    setIsActing(true);
    try {
      if (actionType === 'accept') await api.accept(actionId);
      if (actionType === 'reject') await api.reject(actionId);
      setActionId(null);
      setActionType(null);
      await reload();
    } catch (err) {
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-10">
        ...
        {proposals.map((proposal) => (
          <div key={proposal.id} className="rounded-xl border bg-white p-6">
            ...
            <p className="mt-3 text-sm font-medium text-[#00A86B]">
              {proposal.proposedPrice} · {proposal.estimatedDurationDays} يوم
            </p>
      ...
      <ConfirmDialog
        open={actionType === 'accept'}
        title="قبول العرض"
        message="هل أنت متأكد من قبول هذا العرض؟ سيتم رفض العروض الأخرى المعلقة وتحويل المشروع إلى قيد التنفيذ."
        confirmLabel="قبول"
        isLoading={isActing}
